...
def resolve_transform(node):
    """
    任意のオブジェクト名から最初の transform/joint を返す
    """
...
class IKRigBuilder:
    def __init__(self, chain, pv_offset, upv_locator=None):
        ...
        self.ik_ctrl = None
        self.pv_ctrl = None
        self.cons = [] # すべての一時オブジェクト/コンストレインをクリーンアップするためのリスト
        self.ik_to_fk_orient_cons = [] # IK生成後にFKを追従させるためのオリエントコンストレイン
        self.start = cmds.playbackOptions(q=True, min=True)
        self.end = cmds.playbackOptions(q=True, max=True)

    def create_ik(self):
        # 1. Joint chain from FK controllers
        prev = None
        for i, ctrl in enumerate(self.chain.nodes):
            ...
            if prev:
                ...
                j = cmds.joint(name=name, p=pos)
            else:
                cmds.select(clear=True) # 最初のジョイントは親なしで作成
                j = cmds.joint(name=name, p=pos)
        ...
        pc = cmds.parentConstraint(self.ik_ctrl, self.ikh, mo=True)
        ...
        # 5. IKジョイントからFKコントローラへのOrient Constraint
        # これらのコンストレインはbake時に一時的に削除され、bake後に再作成される
        for ctrl_node, jnt in zip(self.chain.nodes, self.temp_joints):
            oc = cmds.orientConstraint(jnt, ctrl_node, mo=True)[0]
            self.ik_to_fk_orient_cons.append(oc) # 専用リストに追加
            self.cons.append(oc) # 全体クリーンアップリストにも追加

        # 6. Transfer FK animation onto IK
    ...
    def _transfer_fk_animation_to_ik(self):
        ...
        if temp_permanent_cons_to_delete:
            ...
            # self.cons からも削除して、bake後に再作成される際に重複しないようにする
            self.cons = [c for c in self.cons if c not in temp_permanent_cons_to_delete]
            self.ik_to_fk_orient_cons = [] # リストをクリアし、再作成時に再登録

        # 2. 一時コンストレイン (FK -> IK/PV) を作成
        temp_bake_cons = []
        try: # try-finally を使用して、確実に一時コンストレインをクリーンアップ
        ...
        # 5. 削除したIKからFKへのOrient Constraintsを再作成
        # これにより、bake後もFKがIKに追従し続ける
        for ctrl_node, jnt in zip(self.chain.nodes, self.temp_joints):
            oc = cmds.orientConstraint(jnt, ctrl_node, mo=True)[0]
            self.ik_to_fk_orient_cons.append(oc) # 専用リストに再追加
            self.cons.append(oc) # 全体クリーンアップリストにも再追加
    ...
    def _delete_temp(self):
        # self.cons に含まれるすべてのオブジェクトとコンストレインを削除
        for c in list(self.cons): # コピーをイテレートして安全に削除
            if cmds.objExists(c): cmds.delete(c)
        
        # 個別に管理しているIK->FKオリエントコンストレインも確実に削除
        for oc in list(self.ik_to_fk_orient_cons):
            if cmds.objExists(oc): cmds.delete(oc)

        for obj in (self.ikh, self.ik_ctrl, self.pv_ctrl):
            if obj and cmds.objExists(obj): cmds.delete(obj)
        for j in self.temp_joints:
            if cmds.objExists(j): cmds.delete(j)
        if self.upv_locator and cmds.objExists(self.upv_locator):
...
class FKIKSwitcherUI:
    ...
    def show(self):
        ...
        self.window_name = cmds.window(self.window_name, title="FK⇔IK Switcher", widthHeight=(400,350))
        ...
        self.num_ctrl_field = cmds.intSliderGrp(field=True, min=3, max=10, value=self.num_controllers,
            fieldMinValue=3, fieldMaxValue=100, step=1, columnWidth3=(120,50,200), label="Count:",
        ...
        self.upv_field = cmds.textFieldButtonGrp(label="アップベクター元FKコントローラ:",
            buttonLabel="Set", columnWidth=[(1,150),(2,200),(3,50)], parent=main)
        cmds.textFieldButtonGrp(self.upv_field, edit=True, buttonCommand=self._set_upv_field)
        cmds.button(label="アップベクター仮生成", command=self._create_upv_locator, width=120, parent=main)

        cmds.separator(height=10, style='in', parent=main)
        self.pv_offset_field = cmds.floatFieldGrp(numberOfFields=1, label="Pole Vector Offset:", value1=1.0, parent=main)

        cmds.separator(height=10, style='none', parent=main)
        cmds.rowLayout(numberOfColumns=3, columnWidth3=(130,130,130), adjustableColumn=3, parent=main)
        cmds.button(label="Generate IK", command=self._on_generate_ik, width=120)
        cmds.button(label="Bake & Delete IK", command=self._on_bake_and_delete, width=120)
        cmds.button(label="Delete IK Only", command=self._on_delete_ik, width=120)

        cmds.showWindow(self.window_name)

    def _create_controller_fields(self, count):
        for f in self.controller_fields:
            if cmds.control(f, exists=True): cmds.deleteUI(f)
        self.controller_fields = []
        for i in range(count):
            f = cmds.textFieldButtonGrp(label=f"FK Controller {i+1}:", buttonLabel="Select",
                columnWidth=[(1,100),(2,200),(3,50)], parent=self.ctrl_field_layout)
