...
「アップベクター仮生成」ボタン押下時に、まず元となるFKコントローラの位置・回転を持つ「親ロケーター」を作成。

その子として同じ位置・回転の「子ロケーター」を生成し、ユーザーがこの子ロケーターを自由に移動・回転できるようにする。

仮IK生成時のアップベクター参照先変更

IKRigBuilder では、従来の「固定オフセットかPV計算」に加え、UIで作成した子ロケーターのワールド座標を参照してPole Vector位置を決定。

これにより、ユーザーが子ロケーターを動かすと、その動きを即座にIK設定に反映させられる。

Generate IK 完了後のクリーンアップ

ユーザーが「Generate IK」を実行したタイミングで、UI用に生成した親子ロケーター（親・子とも）を自動削除。

シーンに不要な一時ロケーターを残さず、作業をすっきり保つ。

既存機能への影響最小化

既存のPVオフセット計算、IKハンドル生成、Bake & Cleanup／Delete IKのみ機能への影響を避けつつ、上記アップベクター機能をオプション的に追加。

UIコードやIKビルダーの構造変更は最小限に留め、ほかのモジュールやコールバック実装はそのまま保持する。
"""
